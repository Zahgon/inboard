...
// error so handlers and middleware can return it directly to echo.
...
func ErrRender(err error) error {
	return &ErrResponse{
		Err:            err,
		HTTPStatusCode: http.StatusUnprocessableEntity,
...
func ErrValidation(err error, valErr validation.Errors) error {
	return &ErrResponse{
		Err:              err,
		HTTPStatusCode:   http.StatusUnprocessableEntity,
		StatusText:       http.StatusText(http.StatusUnprocessableEntity),
